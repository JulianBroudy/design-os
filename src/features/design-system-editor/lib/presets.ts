/**
 * Tailwind color presets mapped to full shadcn/ui semantic token sets.
 *
 * Each preset takes a Tailwind color family and generates all 19 semantic
 * color roles for both light and dark modes. Neutral roles use the stone palette.
 */

import type { ColorTokens } from '../types'

// ---------------------------------------------------------------------------
// Tailwind v4 oklch color palettes (official values)
// ---------------------------------------------------------------------------
...
export const TAILWIND_PALETTES: Record<string, ColorScale> = {
  stone, slate, zinc,
...
export const PRESET_NAMES = Object.keys(TAILWIND_PALETTES)
...
/**
 * Generate a full semantic color token set from a Tailwind color family.
 *
 * Strategy:
 * - Light mode: use shade 600 as primary, stone for neutrals
 * - Dark mode: use shade 400 as primary, stone dark for neutrals
 * - Destructive always uses rose
 */
export function generatePreset(colorName: string): { light: ColorTokens; dark: ColorTokens } {
  const palette = TAILWIND_PALETTES[colorName]
  if (!palette) {
    return generatePreset('stone')
  }

  // Determine if this is a neutral palette (low chroma)
  const isNeutral = ['stone', 'slate', 'zinc'].includes(colorName)

  // Use the palette for primary colors, stone for neutral backgrounds
  const neutral = isNeutral ? palette : stone
  ...
  return {
    light: {
      ...
      primary: isNeutral ? palette[900] : palette[600],
      'primary-foreground': isNeutral ? palette[50] : 'oklch(1 0 0)',
      ...
      border: neutral[200],
      input: neutral[200],
      ring: isNeutral ? palette[500] : palette[600],
    },
    dark: {
      ...
      primary: isNeutral ? palette[200] : palette[400],
      'primary-foreground': isNeutral ? palette[900] : neutral[950],
      ...
      border: neutral[700],
      input: neutral[700],
      ring: isNeutral ? palette[400] : palette[400],
    },
  }
}

/**
 * Get a representative display color for a palette (for the preset picker grid).
 * Returns the 500 shade as a hex-style oklch value.
 */
export function getPresetDisplayColor(colorName: string): string {
  return TAILWIND_PALETTES[colorName]?.[500] ?? stone[500]
}
